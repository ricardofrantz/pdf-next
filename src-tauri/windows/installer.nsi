...
Function PageReinstall
  ...
  ${Else}
    ...
    ; On an upgrade, start on "don't uninstall": overwrite is enough for this
    ; app, and the first radio is the path that hits the empty `_?=` bug.
    ${If} $R0 = 1
    ${AndIf} $ReinstallPageCheck = 0
...
Function PageLeaveReinstall
  ...
  reinst_uninstall:
    ...
    ${Else}
      ...
      ${If} $4 == ""
        ; No path: `_?=` empty is the NSIS error. Overwrite instead.
        Goto reinst_done
      ${EndIf}
      ReadRegStr $R1 SHCTX "${UNINSTKEY}" "UninstallString"
...
; Fill $4 with the directory of a previous copy. 0.9.0 wrote
; Software\frantz\pdf-next; later publishers write a different key.
; Add/Remove Programs still has InstallLocation under the product name.
Function ResolvePreviousInstallDir
  ReadRegStr $4 SHCTX "${MANUPRODUCTKEY}" ""
  ${If} $4 == ""
    ReadRegStr $4 SHCTX "Software\frantz\${PRODUCTNAME}" ""
  ${EndIf}
  ${If} $4 == ""
    ReadRegStr $4 SHCTX "Software\Ricardo Frantz\${PRODUCTNAME}" ""
  ${EndIf}
  ${If} $4 == ""
